...
fn add_world_options_loc_examine_from_pick() {
    ...
    Arc::get_mut(&mut c.cache).unwrap().locs[1].op =
        vec![Some("Chop".into()), None, None, None, None];
    c.world
...
fn add_world_options_npc_ops_from_pick() {
    ...
    let actions: Vec<i32> = (0..c.menu_num_entries)
        ...
        .collect();
    assert!(
        actions.contains(&MiniMenuAction::OP_NPC2),
        "Talk -> OP_NPC2"
    );
...
fn add_world_options_player_ops_from_pick() {
    ...
    let walk = actions
        .iter()
        .position(|&a| a == MiniMenuAction::WALK)
        .unwrap();
...
fn add_world_options_obj_take_and_examine_from_pick() {
    ...
    let rune = actions
        .iter()
        .position(|&a| a == MiniMenuAction::OP_OBJ6)
        .unwrap();
    assert_eq!(c.menu_option[rune], "Examine @lre@Rune");
    let coins = actions
        .iter()
        .rposition(|&a| a == MiniMenuAction::OP_OBJ6)
        .unwrap();
...
fn add_npc_options_and_add_player_options_callable_directly() {
    ...
    assert!(
        actions.contains(&MiniMenuAction::OP_PLAYER2),
        "player op[1]"
    );
}
...
fn build_minimenu_sorts_1000_plus_below_actions() {
    ...
    assert_eq!(
        ...
        "Examine sinks below the actions"
    );
    assert_eq!(
        c.menu_action[2],
        MiniMenuAction::OP_HELD5,
        "Drop stays last entry"
    );
}
...
fn inv_slot_obj_iop_and_component_iop_options() {
    ...
    assert!(
        actions.contains(&MiniMenuAction::INV_BUTTON1),
        "component iop[0]"
    );
    let held1 = actions
        .iter()
        .position(|&a| a == MiniMenuAction::OP_HELD1)
        .unwrap();
    ...
    let inv1 = actions
        .iter()
        .position(|&a| a == MiniMenuAction::INV_BUTTON1)
        .unwrap();
...
fn inv_slot_use_and_target_replace_ops() {
    ...
    let actions: Vec<i32> = (0..c.menu_num_entries)
        ...
        .collect();
    assert!(
        actions.contains(&MiniMenuAction::USEHELD_ONHELD),
        "Use with"
    );
    assert!(
        !actions.contains(&MiniMenuAction::OP_HELD5),
        "no Drop in use mode"
    );
    assert!(
        !actions.contains(&MiniMenuAction::OP_HELD6),
        "no Examine in use mode"
    );
    ...
    let actions: Vec<i32> = (0..c.menu_num_entries)
        ...
        .collect();
    assert!(
        actions.contains(&MiniMenuAction::TGT_HELD),
        "target with 0x10 mask"
    );
    ...
    assert_eq!(
        c.menu_num_entries, 1,
        "no target option without the 0x10 mask bit"
    );
}
...
fn non_inv_buttons_push_button_actions() {
    ...
    let close = actions
        .iter()
        .position(|&a| a == MiniMenuAction::CLOSE_BUTTON)
        .unwrap();
    assert_eq!(c.menu_option[close], "Close");
    assert_eq!(c.menu_param_c[close], 3);
    let target = actions
        .iter()
        .position(|&a| a == MiniMenuAction::TGT_BUTTON)
        .unwrap();
    assert_eq!(
        c.menu_option[target], "Cast @gre@Tree",
        "prefix is the first word of targetVerb"
    );
}
...
fn paused_and_targeting_suppress_continue_and_target_buttons() {
    ...
    let actions: Vec<i32> = (0..c.menu_num_entries)
        ...
        .collect();
    assert!(
        !actions.contains(&MiniMenuAction::PAUSE_BUTTON),
        "paused latches the Continue"
    );
    assert!(
        !actions.contains(&MiniMenuAction::TGT_BUTTON),
        "targeting replaces TGT_BUTTON"
    );
}
...
fn chat_region_adds_report_abuse_for_staff() {
    ...
    let actions: Vec<i32> = (0..c.menu_num_entries)
        ...
        .collect();
    assert!(
        actions.contains(&MiniMenuAction::ABUSE_REPORT),
        "staff report-abuse"
    );
    let abuse = actions
        .iter()
        .position(|&a| a == MiniMenuAction::ABUSE_REPORT)
        .unwrap();
...
fn chat_line_adds_friend_and_ignore_for_friend() {
    ...
    let actions: Vec<i32> = (0..c.menu_num_entries)
        ...
        .collect();
    assert!(
        actions.contains(&MiniMenuAction::FRIENDLIST_ADD),
        "Add friend"
    );
    assert!(
        actions.contains(&MiniMenuAction::IGNORELIST_ADD),
        "Add ignore"
    );
    let add = actions
        .iter()
    ...
    c2.build_minimenu();
    let actions: Vec<i32> = (0..c2.menu_num_entries)
        .map(|i| c2.menu_action[i as usize])
        .collect();
    assert!(
        !actions.contains(&MiniMenuAction::FRIENDLIST_ADD),
        "non-friend has no Add friend"
    );
}
...
fn social_component_ok_override_pushes_remove_and_message() {
    ...
    assert!(
        !actions.contains(&MiniMenuAction::IF_BUTTON),
        "override suppresses IF_BUTTON"
    );
    assert!(actions.contains(&MiniMenuAction::FRIENDLIST_DEL), "Remove");
    assert!(
        actions.contains(&MiniMenuAction::MESSAGE_PRIVATE),
        "Message"
    );
    let remove = actions
        .iter()
...
fn left_click_add_friend_last_entry_opens_menu() {
    ...
    assert!(
        c.is_menu_open,
        "the add-friend last entry must open the menu"
    );
}
